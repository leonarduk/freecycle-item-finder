...
public class FreecycleScraperTest {
	...
	public final void testCleanHtml() {
		final String detailCandidate = "We have a spare Neue undercounter refrigerator. It is hardly used (1 week only) and in perfect working order. Comes complete with clean white door cover as per pictures. Perfect condition, just looking to sell! "
	...
	@Ignore
	@Test
	public final void testGetFullPostFromFile() throws ParserException {
		final String url = this.getClass().getClassLoader().getResource("mirror.html").toString();
		final HtmlParser parserInstance2 = new HtmlParserImpl(url);

		final FreecycleGroup group = FreecycleGroup.kingston;
		final FreecycleScraper scraper2 = new FreecycleScraper(parserInstance2, group);
