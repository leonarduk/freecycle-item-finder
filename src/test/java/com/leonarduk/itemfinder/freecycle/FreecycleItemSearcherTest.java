...
public class FreecycleItemSearcherTest {
	...
	public final void testFullPost() throws ParserException, IOException {
		final FreecycleGroup kingston = FreecycleGroup.kingston;

		final String fullPostUrl = ClassLoader.getSystemResource("sofa.html").getPath();
		final HtmlParser parser = Mockito.mock(HtmlParser.class); // queryBuilder.build();
		Mockito.when(parser.getURL()).thenReturn(fullPostUrl);
		//
		final FreecycleScraper scraper = new FreecycleScraper(parser, kingston);

		final Post post = new Post(PostType.OFFER, new Date(), "sofa", fullPostUrl, kingston);
