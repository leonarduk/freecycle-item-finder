...
public class Currency implements Comparable {
	...
	private static NumberFormat df = NumberFormat.getNumberInstance(Locale.US); // We
																				// create
																				// a
																				// lot
																				// of
																				...
																				// is
																				// good.
	public static final int NONE = 0, US_DOLLAR = 1, UK_POUND = 2, JP_YEN = 3,
			GER_MARK = 4, FR_FRANC = 5, CAN_DOLLAR = 6;
	public static final int EURO = 7, AU_DOLLAR = 8, CH_FRANC = 9,
			NT_DOLLAR = 10, TW_DOLLAR = 10, HK_DOLLAR = 11;
	public static final int MY_REAL = 12, SG_DOLLAR = 13, IND_RUPEE = 14;
	private static Currency _noValue = null;

	/**
	 * @brief This provides a concept of a currency value that is invalid, not
	 *        just 'zero' in some arbitrary currency.
	 * 
	 * @return A single, consistent, 'Empty Value', which indicates an invalid
	 *         currency.
	 */
	public static Currency NoValue() {
		if (_noValue == null)
			_noValue = new Currency(NONE, 0.0);
	 ...
	 *
	 * Takes two values (usd, non-usd) which are believed to be equivalent, and
	 * a currency amount to convert based off the ratio between the first two.
	 * If the USD amount is null or $0, it looks in a table it keeps around for
	 * converting. If that fails, it just returns the non-USD's value.
	 *
	 * @param usd
	 *            - A sample US dollar amount.
	 * @param nonusd
	 *            - A non-US dollar amount that is equivalent to the usd
	 *            paramter.
	 * @param cvt
	 *            - The non-USD amount to convert to USD.
	 *
	 * @return - 'cvt' converted by the ratio of usd:nonusd, or by an internal
	 *         table if it couldn't figure out the ratio, or just the non-usd's
	 *         amount as a USD amount if there wasn't even an entry in the
	 *         table.
	 */
	public static Currency convertToUSD(Currency usd, Currency nonusd,
			Currency cvt) {
		if (cvt != null && !cvt.isNull() && cvt.getCurrencyType() != US_DOLLAR) {
			double multiple;
			if (usd == null || usd.isNull() || usd.getValue() == 0.0
					|| nonusd == null || nonusd.isNull()
					|| nonusd.getValue() == 0.0) {
				if (sCurrencyMap.containsKey(cvt.getCurrencyType())) {
					multiple = sCurrencyMap.get(cvt.getCurrencyType());
				} else {
					// If we have nothing else to go on, treat it as exactly
					// equal to USD.
					multiple = 1.0;
				}
			} else {
	...
	private static final Object xlateTable[][] = { { "USD", CurDollar },
			{ "US $", CurDollar }, { "AU $", CurAu }, { "au$", CurAu },
			{ "AU", CurAu }, { "AUD", CurAu }, { "US", CurDollar },
			{ "USD $", CurDollar }, { "$", CurDollar }, { "C", CurCan },
			{ "C $", CurCan }, { "CAD", CurCan }, { "c$", CurCan },
			{ "GBP", CurPound }, { objPound.toString(), CurPound },
			{ "pound", CurPound }, { "\u00A3", CurPound },
			{ "&pound", CurPound }, { "Y", CurYen }, { "JPY", CurYen },
			{ "&yen", CurYen }, { "\u00A5", CurYen }, { "DM", CurMark },
			{ "FRF", CurFranc }, { "fr", CurFranc }, { "CHF", CurSwiss },
			{ "chf", CurSwiss }, { "dm", CurMark }, { "\u20AC", CurEuro },
			{ "eur", CurEuro }, { "EUR", CurEuro }, { "Eur", CurEuro },
			{ "NT$", CurTaiwan }, { "nt$", CurTaiwan }, { "NTD", CurTaiwan },
			{ "HK$", CurHK }, { "hk$", CurHK }, { "HKD", CurHK },
			{ "MYR", CurMyr }, { "myr", CurMyr }, { "SGD", CurSGD },
			{ "sgd", CurSGD }, { "INR", CurRupee }, { "inr", CurRupee } };

	/**
	 * @brief Convert from a string containing a recognized symbol into a
	 *        currency type.
	 * 
	 * @param symbol
	 *            - The string representation of a currency.
	 * 
	 * @return - The integer value associated with the provided currency, or
	 *         NONE for unrecognized currencies.
	 */
	...
	public static Currency getCurrency(String wholeValue) {
		if (wholeValue == null || wholeValue.length() == 0
				|| wholeValue.startsWith("UNK"))
			return NoValue();
	...
	private int checkLengthMatchStart(String value, String currencyName) {
		...
		if (lowVal.startsWith(curNam)) {
			...
			while (len < value.length()
					&& !Character.isDigit(value.charAt(len)))
				len++;
			return len;
		}
	...
	/**
	 * @brief Provided an entire string containing a currency prefix and an
	 *        amount, extract the two and set this object's value to equal the
	 *        result.
	 *
	 *        Is there a reason this doesn't use xlateSymbolToType? BUGBUG --
	 *        mrs: 03-January-2003 01:28
	 * 
	 * @param wholeValue
	 *            - The string containing an entire currency+amount text.
	 */
	private void setValues(String wholeValue) {
		if (wholeValue == null || wholeValue.equals("null")) {
			...
			if (wholeValue.startsWith("US $")) {
				...
				// In case eBay ever corrects to the RIGHT currency code for
				// USD.
				parseCurrency = "USD $";
				...
				if (!isDigit(firstChar) && firstChar != '$') {
					...
					if (semiIndex != -1) {
						...
						valuePortion = wholeValue.substring(parseCurrency
								.length() + 1);
					} else {
						parseCurrency = "$";
			...
			while (valuePortion.length() != 0
					&& !Character.isDigit(valuePortion.charAt(0)))
	...
	/**
	 * @brief If it's set as two seperate entries, then we use the MUCH cleaner
	 *        xlateSymbolToType function.
	 *
	 *        This should be the basic method that setValues works also.
	 * 
	 * @param symbol
	 *            - The string form of a currency symbol.
	 * @param startValue
	 *            - The amount associated with the currency.
	 */
	...
	/**
	 * @brief Get the full, storable textual name for the currency type of this
	 *        object.
	 * 
	...
	/**
	 * @brief Add two currencies and return a new currency containing the result
	 *        of the two added together.
	 * 
	 * @param addValue
	 *            - The currency value/amount to add. It must be of the same
	 *            currency type as 'this'.
	 * 
	 * @return A new currency object containing the sum of the two amounts
	 *         provided, with the same currency type as them.
	 *
	 * @throws CurrencyTypeException
	 *             if the two objects are of different currencies.
	 */
	public Currency add(Currency addValue) throws CurrencyTypeException {
		...
		// If only one currency is known, return the result as the known
		// currency.
		if (mCurrencyType == NONE)
			return new Currency(addValue.getCurrencyType(), mValue
					+ addValue.getValue());
		if (addValue.getCurrencyType() == NONE)
			return new Currency(mCurrencyType, mValue + addValue.getValue());

		throw new CurrencyTypeException("Cannot add " + fullCurrencyName()
				+ " to " + addValue.fullCurrencyName() + ".");
	}

	/**
	 * @brief Subtract two currencies and return a new currency containing the
	 *        result of the passed value subtracted from this objects value.
	 *
	 * @param subValue
	 *            - The currency value/amount to subtract. It must be of the
	 *            same currency type as 'this'.
	 *
	...
	public Currency subtract(Currency subValue) throws CurrencyTypeException {
		...
		// currency.
		if (mCurrencyType == NONE)
			return new Currency(subValue.getCurrencyType(), mValue
					- subValue.getValue());
		if (subValue.getCurrencyType() == NONE)
			return new Currency(mCurrencyType, mValue - subValue.getValue());

		throw new CurrencyTypeException("Cannot subtract " + fullCurrencyName()
				+ " from " + subValue.fullCurrencyName() + ".");
	}
	...
	/**
	 * @brief Format the currency and amount as appropriate for the current
	 *        locale.
	 *
	 *        This is kind of interesting, because it will display in one
	 *        fashion, but when it snipes or bids, it's all against the US
	 *        sites, so it's all operating in US forms at that point.
	 * 
	 * @return A nicely formatted, locale-correct money value, prefixed with the
	 *         best currency symbol for the currency type.
	 */
	public String toString() {
		if (isNull()) {
			return ("null");
	 ...
	 * 
	 * @return - The hash code of the string consisting of the full currency
	 *         named followed by the value as a string. Null/invalid currency
	 *         entries return 0.
	 */
	public int hashCode() {
		if (isNull())
	 ...
	 * 
	 * @return True if the two values are the same, or the currency and amount
	 *         are the same. False otherwise, including false if it is an
	 *         entirely different class. Differing currencies are always
	 *         unequal.
	 */
	 ...
	 *            - The value to compare against.
	 * 
	 * @return - True if this amount is less than the otherValue amount and both
	 *         currency types are equal. If the otherValue is null, the same
	 *         object as this (this.less(this)), or this amount is actually
	 *         less, then it returns false.
	 *
	 * @throws CurrencyTypeException
	 *             if you try to compare different currencies.
	 */
	public boolean less(Currency otherValue) throws CurrencyTypeException {
		...
		boolean lowerValue = Double.compare(
				(double) ((int) (otherValue.getValue() * 1000)),
				(double) (int) (mValue * 1000)) == 1;
	 ...
	 *
	 *        It should probably check against the invalid currency object
	 *        first...
	 * 
	...
	/**
	 * @brief The comparable interface defines this, and so I'm comparing using
	 *        the well defined set of rules for Comparables.
	 *
	 ...
	 * 
	 * @return -1 if o's class is Currency, it's the same currency type, and the
	 *         amount of this is less than o's amount. 0 if o's class is
	 *         Currency, it's the same currency type, and the amount of this is
	 *         the same as o's amount. 1 if o's class is Currency, it's the same
	 *         currency type, and the amount of this is greater than o's amount.
	 *
	 * @throws ClassCastException
	 *             if you try to compareTo non-Currency classes.
	 */
	public int compareTo(Object o) {
		...
		if (!(o instanceof Currency))
			throw new ClassCastException(
					"Currency cannot compareTo different classes!");
		...
		try {
			...
			// Can't re-throw (or not catch!) because Object.compareTo doesn't
			// throw CurrencyTypeException!
			throw new ClassCastException(
					"Currency cannot compareTo different currencies!\n" + e);
		}
	...
	public static PersistenceDelegate getDelegate() {
		return new DefaultPersistenceDelegate(new String[] { "mCurrencyType",
				"mValue" });
	}
}
