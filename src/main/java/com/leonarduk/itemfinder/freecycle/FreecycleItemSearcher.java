...
public class FreecycleItemSearcher implements ItemSearcher {
	 ...
	 */
	@Override
	public final Set<Item> findItems(final QueryBuilder queryBuilder, final Integer limit)
	...
	private final Set<Item> getPosts(final HtmlParser parser, final QueryBuilder queryBuilder,
		...
		for (final Post post : posts) {
			...
			this.log.info("Process " + post.getText());
	 ...
	 * @return the latest post
	 */
	private LatestPost saveLatestPost(final FreecycleGroup groups, final LatestPost latest) {
	...
	private final boolean shouldBeReported(final Post post, final LatestPost latest) {
		...
		this.log.info("report?: " + shouldBeReported + " ID: " + post.getPostId() + " Last: "
