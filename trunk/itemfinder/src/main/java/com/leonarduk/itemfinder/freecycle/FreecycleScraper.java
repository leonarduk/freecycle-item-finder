...
public class FreecycleScraper {
    ...
    private static final NodeFilter TABLE_LINK_FILTER =
            new AndFilter(new TagNameFilter("td"), new HasChildFilter(
                    new TagNameFilter("a")));
     ...
     *             the parse exception
     */
    private static Date parseDateFrom(
            final Node typeAndDateNode,
    ...
    public FreecycleScraper(final HtmlParser parserInstance,
        ...
        this.freecycleDateFormat =
                new SimpleDateFormat(FreecycleScraper.DATE_FORMAT);
    ...
    public final FreecycleItem getFullPost(final Post post)
        ...
        final NodeList thumbnailNodes =
                this.getParser().extractAllNodesThatMatch(
        ...
        final FreecycleItem details =
                new FreecycleItem(post.getLink(), location, post.getText(),
    ...
    private NodeList getHTMLNodes() throws ParserException {
        ...
        return this.parser
                .extractAllNodesThatMatch(FreecycleScraper.TABLE_LINK_FILTER);
    }
    ...
    public final List<Post> getPosts() {
        ...
        try {
            ...
            FreecycleScraper.LOG.info(String.format(
                    "Parsed %s matching HTML nodes", list.size()));
            ...
            while (iterator.hasMoreNodes()) {
                try {
                    ...
                    FreecycleScraper.LOG.error(
                            "Unable to parse HTML node into a post", e);
                }
            }
        }
        ...
        FreecycleScraper.LOG.info(String.format("Returning %s posts",
                this.posts.size()));
        return this.posts;
    }
     ...
     *             the parse exception
     */
    private Post parsePostFromHTMLNodes(final SimpleNodeIterator iterator)
            throws ParseException {
        final Node typeAndDateNode = iterator.nextNode();
        final PostType postType = PostType.parse(typeAndDateNode);
        final Date postDate =
                FreecycleScraper.parseDateFrom(typeAndDateNode,
                        this.freecycleDateFormat);
